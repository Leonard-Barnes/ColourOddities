...
public class CameraMovement : MonoBehaviour
{
  ...
  private void CameraZoom()
  {
    ...
    if (bounds.size.y > bounds.size.x && bounds.size.y > 3f)
    {
      float diff = zoom - bounds.size.y;
      if(-2 <= diff && diff <= 2)
      {
        zoom = bounds.size.y;
      }
      newZoom = Mathf.MoveTowards(minZoom, maxZoom, zoom / (zoomLimiter / 1.5f));
      lerpZoom = Mathf.Lerp(cam.orthographicSize, newZoom, cameraSpeed / 4);
      cam.orthographicSize = lerpZoom;
    }
    else
    {
      float diff = zoom - bounds.size.x;
      if(-2 <= diff && diff <= 2)
      {
        zoom = bounds.size.x;
      }
      newZoom = Mathf.MoveTowards(minZoom, maxZoom, bounds.size.x / zoomLimiter);
      lerpZoom = Mathf.Lerp(cam.orthographicSize, newZoom, cameraSpeed / 4);
      cam.orthographicSize = lerpZoom;
    }
  }
