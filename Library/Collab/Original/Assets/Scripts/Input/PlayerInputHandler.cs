...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    {
        RaycastHit2D raycastHit = Physics2D.BoxCast(box.bounds.center, box.bounds.size, 0f, Vector2.right, extraHeight /2, platformMask);
        return raycastHit.collider != null;
    }

    private bool isWalledLeft()
    {
        RaycastHit2D raycastHit = Physics2D.BoxCast(box.bounds.center, box.bounds.size, 0f, Vector2.left, extraHeight /2, platformMask);
        return raycastHit.collider != null;
    }
    ...
    {
        if (rb.velocity.y > 0.1f && isWalledAbove() == false)
        {
        ...
        if (rb.velocity.y < -0.05f & isGrounded() && isWalledAbove() == false)
        {
